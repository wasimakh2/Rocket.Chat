export class AppPersistenceBridge {
	...
	async purge(appId) {
		...
		this.orch.getPersistenceModel().remove({ appId: appId });
	}
	...
	async createWithAssociations(data, associations, appId) {
		...
		return this.orch.getPersistenceModel().insert({ appId: appId, associations: associations, data: data });
	}
	...
	async update(id, data, upsert, appId) {
		...
		throw new Error('The update function is not implemented.');
	}
	...
	const records = this.orch.getPersistenceModel().find({ appId: appId, associations: { $all: associations } }).fetch();
	...
	const query = { appId: appId, associations: { $all: associations } };
